...
import static tool.probfuzz.Utils.STANRUNNER;
        ...
        for (Section section : sections) {
            if (section.sectionType == SectionType.DATA) {
                ...
                if (section.sectionName.equals("main")) {
                    visitBlocks(section.basicBlocks);
...
//                    }
    ...
    public void visitBlocks(ArrayList<BasicBlock> basicBlocks){
        ...
        while(!queue.isEmpty()){
            ...
            if(block_text.trim().length() > 0) {
                if (cur.getIncomingEdges().containsKey("true")) {
                    block_text = "{\n" + block_text;
                }

                if (cur.getIncomingEdges().containsKey("false") && !cur.getIncomingEdges().containsKey("meet")) {
                    block_text = "{\n" + block_text;
                ...
                if (cur.getOutgoingEdges().containsKey("back") || cur.getOutgoingEdges().containsKey("meet")) {
                    block_text = block_text + "}\n";
                }
    ...
    private String translate_block(BasicBlock bBlock) {
        ...
        for (Statement statement : bBlock.getStatements()) {
            if (statement.statement instanceof AST.AssignmentStatement) {
                ...
                if (CommonUtils.isPrior(statement, assignmentStatement.lhs) || isData(statement, assignmentStatement.lhs)) {
                    String newAssign;
                ...
                if (statement.parent.getParent().sectionName.equalsIgnoreCase("main") && CommonUtils.isPrior(statement, declaration.id)) {
...
//        }
    ...
    public Pair run(String codeFileName, String dataFileName) {
        ...
        Pair results = CommonUtils.runCode(codeFileName, dataFileName, STANRUNNER);
        return results;
    }
}
